...
angular.module('myApp.controllers', [])
	...
	function($scope, $location, googleLoginApi) {
			...
			$scope.login = function() {
				if($scope.loggingInUser.email == "tresa.e@gmail.com" && 
	...
	function ($scope, $filter, $parse, googleLoginApi, googleCalendarApi) {
			...
			$scope.$watch('entryInput', function(val) {
				$scope.newEntry = $filter('parseEntry')(val, $scope.users || {});
			});

			var refreshEvents = function() {
				googleCalendarApi.getEventsForCalendar('todoit').then(function(d){
					$scope.events = d;
				});
			}
			refreshEvents();
